...
def _empty(currency: str, msg: str | None = None, dark: bool = True) -> go.Figure:
    ft = theme.fig_theme(dark)
    if msg is None:
        msg = t("No transactions")
    fig = go.Figure()
    fig.update_layout(
        template=ft.template, yaxis_title=f"{t('Amount')} ({currency})",
...
def build_money_flow_figure(
    ...
    for i, account in enumerate(sorted(data["Account"].unique())):
        ...
        fig.add_trace(go.Bar(
            ...
            hovertemplate=(
                ...
                + (t("Amount") + ": ***** " + currency + "<br>"
                   + t("Balance after") + ": ***** " + currency + "<extra></extra>"
                   if censor else
                   t("Amount") + ": %{customdata[1]:,.0f} " + currency + "<br>"
                   + t("Balance after") + ": %{customdata[2]:,.0f} " + currency
                   + "<extra></extra>")
            ),
        ))
    ...
    if forecast:
        ...
        fig.add_trace(go.Scatter(x=fx, y=forecast["lo90"], mode="lines",
                                 ...
                                 name=t("Forecast 90%"), hoverinfo="skip"))
        ...
        fig.add_trace(go.Scatter(x=fx, y=forecast["lo50"], mode="lines",
                                 ...
                                 name=t("Forecast 50%"), hoverinfo="skip"))
        fig.add_trace(go.Scatter(
            x=fx, y=forecast["median"], mode="lines", name=t("Forecast"),
            line=dict(color=fc_color, width=2, dash="dash"),
            hovertemplate="%{x|%d/%m/%Y}<br>"
                          + ("***** " if censor else "%{y:,.0f} ") + currency
                          + "<extra>" + t("Forecast") + "</extra>",
        ))
    ...
    fig.add_hline(y=net_worth, line=dict(color=ft.ink, width=1.2, dash="dash"),
                  annotation_text=f"{t('Net worth')} {nw_txt} {currency}",
    ...
    if not compact:
        ...
        hidden_line = (f"<br>{t('Hidden cost (untracked)')}: {hidden_txt} {currency}"
                       if hidden else "")
        box = (f"<b>{t('Latest balances')}</b><br>{lines}{hidden_line}"
               f"<br><b>{t('Net worth')}: {nw_txt} {currency}</b>")
        fig.add_annotation(
    ...
    fig.update_layout(
        ...
        yaxis_title=(f"{t('Amount')} ({currency})" if not censor else ""),
