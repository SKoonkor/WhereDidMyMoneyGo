...
def build_hist_single(df: pd.DataFrame, start: str, end: str, txn_type: str,
                      currency: str = "THB", dark: bool = True,
                      expense_order: str = "amount",
                      censor: bool = False) -> go.Figure:
    """One bar chart ("histogram") for a single side — ``txn_type`` is "Income" or
    "Expense" — with one bar per category, reusing the pies' breakdowns and colour
    scheme so each bar matches its slice (Income Greens; Expense Reds, or Needs=Blues /
    Wants=Oranges in bucket mode; hidden cost slate). Income and Expense are built as
    two separate figures so narrow screens can stack them (Income above Expense).

    Interaction is locked (both axes ``fixedrange``, ``dragmode=False``): no zoom or
    pan — a tap/hover just shows the category and its amount. Amounts are masked under
    privacy."""
    ft = theme.fig_theme(dark)
    end_incl = pd.Timestamp(end).normalize() + pd.Timedelta(days=1) - pd.Timedelta(seconds=1)
    data = filter_by_date(df, start, str(end_incl))
    ...
    if txn_type == "Income":
        frame = _category_breakdown(data, "Income")
        palette = "Greens"
        hidden = data.loc[data["Income/Expense"] == "Adjustment-In", "Amount"].sum()
        bucket = False
    else:
        frame = (_expense_bucket_breakdown(data, B.load_budget().get("assignments", {}))
                 if bucket_mode else _category_breakdown(data, "Expense"))
        palette = "Reds"
        hidden = data.loc[data["Income/Expense"] == "Adjustment-Out", "Amount"].sum()
        bucket = bucket_mode
    ...
    fig.update_layout(
        template=ft.template, showlegend=False,
        paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(0,0,0,0)",
        margin=dict(t=48, b=100, l=40, r=20),
        dragmode=False,   # no pan / box-zoom; tap a bar to read its value
        title=dict(text=f"<b>{t(txn_type)}</b>", x=0.5, xanchor="center",
                   y=0.98, yanchor="top", font=dict(size=15, color=ft.ink)),
    )
    # fixedrange on both axes locks zoom/pan while leaving hover/tap tooltips.
    ...
    fig.update_yaxes(title_text=currency, showticklabels=not censor,
                     gridcolor=ft.grid, zeroline=False, fixedrange=True)
    return fig
